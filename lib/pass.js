...
function signManifest(template, manifest, callback) {
  ...
  // tell openssl to use /tmp for its working directory to create the .rnd file
  var env = { HOME: "/tmp" };

  var sign = execFile("openssl", args, { stdio: "pipe", env: env }, function(error, stdout, stderr) {
